...
하지만 데이터를 분석하면서, 각 입력에는 `available_actions`가 정확히 3개만 주어졌고, 정답은 항상 그 세 후보 안에 있다는 것을 알게 됐습니다.
...
더 복잡한 모델을 추가하기 전에 문제가 가진 구조적 제약을 먼저 활용해 보았습니다.
...
local validation에서는 Transformer 계열도 높은 성능을 보였지만 hidden 평가에서는 그렇지 않았습니다.
...
특정 행동 class에 예측이 과도하게 몰리는 문제를 완화하기 위해 class별 score bias를 조정하는 **Balanced Assignment**를 적용했습니다. 

이를 통해 hidden Macro F1이 **0.80889 → 0.88347**로 개선됐고, 이후 smoothing을 조정해 **Hidden Macro F1 0.89225**까지 개선했습니다.
...
따라서 최종 제출에서는 F1이 조금 높은 ensemble 대신 전체 expected score가 더 높은 **단일 경량 모델**을 선택했습니다. 
...
대회 환경과 내부 측정 기준에서 **1,440개 sample 추론 약 0.27초**를 기록했고, package 크기는 약 6.77MB였습니다. 추가 heavyweight inference dependency도 최소화했습니다. 
